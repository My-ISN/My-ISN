...
class _RentPlanDetailPageState extends State<RentPlanDetailPage> {
  ...
  final List<String> _menuTabs = [
    'OVERVIEW', 'INVOICE', 'VIEW DOKUMEN', 'PERJANJIAN SEWA'
  ];
      ...
      child: Row(
        children: _menuTabs.map((tab) {
          ...
          return Padding(
            ...
            child: InkWell(
              onTap: () {
                if (['INVOICE', 'PERJANJIAN SEWA'].contains(tab)) {
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(
                    color: isActive ? _primaryColor : Theme.of(context).dividerColor.withOpacity(0.1)
                  ),
                ),
                child: Text(
                  tab,
  ...
  Future<void> _launchDocumentUrl(String tab) async {
    String endpoint = '';
    if (tab == 'INVOICE') endpoint = 'invoice';
    else if (tab == 'PERJANJIAN SEWA') endpoint = 'agreement';
  ...
  Widget _buildOverviewTab() {
    ...
    return Column(
      children: [
        _buildSectionTitle(Icons.laptop_rounded, 'DETAIL RENTAL'),
        _buildOverviewCard([
          _buildRowTwoFields('LAPTOP', _rentalData!['nama_laptop'] ?? '-', 'INVOICE', _rentalData!['invoice_number'] ?? '-'),
          _buildRowTwoFields('JUMLAH UNIT', '${_rentalData!['total_laptop'] ?? 0} unit', 'DURASI', '${_rentalData!['lama_sewa'] ?? 0} hari'),
          _buildRowTwoFields('MULAI', _rentalData!['invoice_date'] ?? '-', 'SELESAI', _rentalData!['tanggal_berakhir'] ?? '-'),
        ]),

        const SizedBox(height: 20),

        _buildSectionTitle(Icons.payments_rounded, 'PEMBAYARAN & STATUS'),
        _buildOverviewCard([
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('TOTAL TAGIHAN', style: TextStyle(fontSize: 10, color: Colors.grey[600], fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
          ...
          _buildRowTwoFields(
            'PEMBAYARAN', 
            _rentalData!['status_pembayaran'] == 'sudah' ? 'Lunas' : 'Belum Lunas',
            'STATUS RENTAL', 
            _getRentalStatusLabel(_rentalData!['status'] ?? '-'),
            valColor1: _rentalData!['status_pembayaran'] == 'sudah' ? Colors.green : Colors.red,
  ...
  Widget _buildViewDokumenTab() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildSectionTitle(Icons.badge_rounded, 'DOKUMEN SAYA'),
            ...
            child: InteractiveViewer(
              ...
              child: Image.network(
                ...
                loadingBuilder: (context, child, loadingProgress) {
                  ...
                  return Container(height: 200, color: Colors.grey[50], child: const Center(child: CircularProgressIndicator()));
                },
              ),
            ),
          ),
