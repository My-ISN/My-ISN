...
class _PayrollPageState extends State<PayrollPage>
  ...
  Future<void> _fetchAccounts() async {
    try {
      ...
      final url =
          '${AppConstants.baseUrl}/get_payroll_accounts?user_id=$userId';
      final response = await http.get(Uri.parse(url));
  ...
  Future<void> _fetchPreview() async {
    ...
    try {
      ...
      final url =
          '${AppConstants.baseUrl}/get_payroll_preview?staff_id=$_selectedStaffId&salary_month=$monthStr&user_id=$userId';
      final response = await http.get(Uri.parse(url));
      final data = json.decode(response.body);
      if (data['status'] == true && mounted) {
        setState(() {
  ...
  Future<void> _executePayment() async {
    ...
    try {
      ...
      final url = '${AppConstants.baseUrl}/execute_payroll_payment';
  ...
  Future<void> _fetchPayrollHistory() async {
    ...
    try {
      final userId = widget.userData['id'] ?? widget.userData['user_id'];
      final url =
          '${AppConstants.baseUrl}/get_payroll_history?user_id=$userId';
      final response = await http.get(Uri.parse(url));
      final data = json.decode(response.body);
  ...
  Future<Map<String, dynamic>> _fetchPayslipDetails(String payslipId) async {
    final userId = widget.userData['id'] ?? widget.userData['user_id'];
    final url =
        '${AppConstants.baseUrl}/get_payslip_details?payslip_id=$payslipId&user_id=$userId';
    final response = await http.get(Uri.parse(url));
