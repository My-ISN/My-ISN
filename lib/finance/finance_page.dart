...
class _FinancePageState extends State<FinancePage>
  ...
  bool _hasPermission(String resource) {
    ...
    bool hasMobileView =
        resourceList.contains('mobile_finance_view') ||
        resourceList.contains('mobile_finance_enable');
    bool hasMobileAddEdit =
        resourceList.contains('mobile_finance_add') ||
        resourceList.contains('mobile_finance_enable');
    bool hasMobileDelete =
        resourceList.contains('mobile_finance_delete') ||
        resourceList.contains('mobile_finance_enable');

    if (resource == 'finance6' ||
        resource == 'finance5' ||
        resource == 'finance1' ||
        resource == 'hr_finance') {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _monthlyExpense =
              (dashboard['data']['monthly_expense'] as num).toDouble();
          _monthlyDeposit =
              (dashboard['data']['monthly_deposit'] as num).toDouble();
          _isLoading = false;
        });
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'finance.delete_account_confirm_msg'.tr(
                context,
                args: {'name': account['account_name'] ?? ''},
              ),
  ...
  Future<void> _performDeleteTransaction(String transactionId) async {
    setState(() => _isTransactionsLoading = true);
    try {
      final response = await _financeService.deleteFinanceTransaction(
        transactionId,
      );
      if (response['status'] == true) {
        if (mounted) {
        ...
        throw Exception(
          response['message'] ??
              'finance.transaction_deleted_failed'.tr(context),
        );
      }
    } catch (e) {
      if (mounted) {
            ...
            if (transaction['attachment_url'] != null &&
              ...
              ClipRRect(
                ...
                child:
                    ...
                    ? InkWell(
                        onTap: () => launchUrl(
                          Uri.parse(transaction['attachment_url']),
                        ...
                        errorWidget: (context, url, error) => Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              const SizedBox(width: 12),
                              Text(
                                'finance.failed_load_image'.tr(context),
  ...
  Future<void> _launchTransactionWebUrl(
    ...
    final Uri url = Uri.parse(
      '${AppConstants.serverRoot}/erp/transaction-details/$encodedId?token=$token',
    );
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Text(
                  'finance.account_details'.tr(context),
                  style: const TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                  ),
                      ...
                      PopupMenuItem(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 12),
                            Text(
                              'finance.delete_account'.tr(context),
                              style: const TextStyle(color: Colors.red),
                            ),
                          ],
                        ),
            ...
            _buildDetailRow(
              Icons.account_balance_wallet_rounded,
              'finance.account_name'.tr(context),
  ...
  Future<void> _launchAccountWebUrl(Map<String, dynamic> account) async {
    ...
    final Uri url = Uri.parse(
      '${AppConstants.serverRoot}/erp/account-ledger/$encodedId?token=$token',
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton:
          (!_hasPermission('finance2') ||
              (_availableTabs.isNotEmpty &&
                  _availableTabs[_tabController.index]['index'] == 3))
          ? null
          : FloatingActionButton.extended(
              onPressed: () async {
                ...
                if (initialType > 2) {
                  initialType = 1; // Default to Income if on 'All' tab
                }
  ...
  Widget _buildTabBar() {
    ...
    return Container(
      ...
      child: TabBar(
        ...
        tabs: _availableTabs
            .map((tab) => Tab(text: (tab['title'] as String).tr(context)))
            .toList(),
      ),
    );
  }
    ...
    return Column(
      children: [
        ...
        if (_accounts.isNotEmpty &&
            _accountsTotalCount > _accountsSelectedLimit)
          Padding(
